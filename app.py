...
def search_movies(genre: str) -> str:
    '''Search for Indian movies by genre.''' # Changed to triple single-quotes
    movies = {
...
def change__to_f(temp_c: float) -> float:
  '''converts the cel temp to F temperature''' # Changed to triple single-quotes
  return temp_c * (1.8) + 32

@tool
def get_weather(city: str) -> str:
    '''Get current temperature for a given city name.''' # Changed to triple single-quotes
